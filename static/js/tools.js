...
/*
-----------------------------------------------------
	Youtube Headers and stadistic Data
-----------------------------------------------------
*/
function getYoutubeId(link){
	...
	if (videoid.match(/^[a-z0-9_-]{11}$/i) === null) {
		console.warn('[getYoutubeId] Invalid Link: '+link);
		return false;
	}else{return videoid;}
}

function getYoutubeEstadistics(link,cors=false,callback=''){
	var videoid=getYoutubeId(link);
	let response=false;
	let status={};
	let timer=(new Date).getTime();
	if(callback==''){callback=console.info;}
	if(videoid==false){console.warn("[getYoutubeEstadistics] Invalid Id.");return videoid;};

	var parts='snippet,contentDetails,statistics,status,topicDetails,recordingDetails,liveStreamingDetails,localizations'
	fetch((cors?'https://cors-anywhere.herokuapp.com/':'')+"https://www.googleapis.com/youtube/v3/videos?key=AIzaSyCVE1HOTB9J_BG5RCK0PNXIZRibxKOmcoM&part="+parts+"&id="+videoid, {
			method: 'GET',
			headers: {
		...
		res.text().then(data=>{
			...
			if (data==undefined||data.items==undefined||data.items.length === 0) {
				status.response=404
				status.description='[getYoutubeEstadistics] Video "'+videoid+'" not found.',data;
				status.error=404;
				status.timer=(new Date).getTime()-timer;
		...
		status.response=500;
		status.description='[getYoutubeEstadistics] Video "'+videoid+'" Fetch Error: '+err;
		status.error=err;
		status.timer=(new Date).getTime()-timer;
		console.error(status.description,status);
		callback(response,status);
